...
if (isset($_REQUEST['getQuocGia'])){
    ...
    foreach ($quocGias as $item){
        $result1 = array('id' => $item->getID(), 'text' => $item->getTen());
...
if (isset($_REQUEST['getSelectedQuocGia']) && isset($_REQUEST['idphim'])){
    $quocGias = $controllerQuocGia->getQuocGiaFromPhim($_REQUEST['idphim']);
    $result = array();
    foreach ($quocGias as $quocGia) {
        $result1 = array('id' => $quocGia->getID(), 'text' => $quocGia->getTen());
        array_push($result, $result1);
    }
    $result = array('quocgia'=>$result);
    echo json_encode($result);
