...
if (isset($_REQUEST['getTheLoai'])){
    ...
    foreach ($theLoais as $item){
        $result1 = array('id' => $item->getID(), 'text' => $item->getTenTheLoai());
...
if (isset($_REQUEST['getSelectedTheLoai']) && isset($_REQUEST['idphim'])){
    $theLoais = $controllerTheLoai->getTheLoaiFormPhim($_REQUEST['idphim']);
    $result = array();
    foreach ($theLoais as $theLoai) {
        $result1 = array('id' => $theLoai->getID(), 'text' => $theLoai->getTenTheLoai());
        array_push($result, $result1);
    }
    $result = array('theloai'=>$result);
    echo json_encode($result);
